...
int monthly_rate(double hourly_rate, double discount) {
    ...
    double pre_month_rate { daily_rate(hourly_rate) * 22 };

    return int( ceil(apply_discount(pre_month_rate, discount)) );
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    // this amount of money corresponds to 22 days
    int cur_monthly_rate { monthly_rate(hourly_rate, discount) };

    return floor( budget * 22/cur_monthly_rate );
}
